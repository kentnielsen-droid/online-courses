...
name, authenticator_status, username = authenticator.login("Login", "sidebar")
...
def display_group_metrics(df, num_clusters):
    for i in range(num_clusters):
        ...
        marital_percentage = (
            round(len(f_df[f_df["Marital status"] == 0]) / len(f_df), 2) * 100
        )
        ...
        university = (
            round(
                len(f_df[(f_df["Education"] == 2) | (f_df["Education"] == 3)])
                / len(f_df),
                2,
            )
            * 100
