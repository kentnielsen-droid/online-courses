...
def plot_volume(ids):
    ...
    for id in ids:
        ax.plot(x, df[df["parts_id"] == id]["volume"], label=id)
...
def format_dataset(ids):
    ...
    model_df.rename(
        {"parts_id": "unique_id", "date": "ds", "volume": "y"}, axis=1, inplace=True
    )
...
def create_sf_object(model_df):
    ...
    sf = StatsForecast(df=model_df, models=models, freq="MS", n_jobs=-1)
...
if __name__ == "__main__":
    ...
    product_ids = st.multiselect("Select product ID", options=df["parts_id"].unique())
